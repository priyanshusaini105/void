...
export class Chat extends Base {
    ...
    this._symmetryService?.on(
      ...
      (completion: string) => {
        this._webView?.postMessage({
          type: EVENT_NAME.voidOnCompletion,
          data: {
  ...
  private async getRelevantFiles(
    ...
    if (await this._db.hasEmbeddingTable(table)) {
      ...
      const relevantFileCountContext = `${EVENT_NAME.voidGlobalContext}-${EXTENSION_CONTEXT_NAME.voidRelevantFilePaths}`
  ...
  private getRerankThreshold() {
    const rerankThresholdContext = `${EVENT_NAME.voidGlobalContext}-${EXTENSION_CONTEXT_NAME.voidRerankThreshold}`
    const stored = this.context?.globalState.get(
  ...
  private async getRelevantCode(
    ...
    if (await this._db.hasEmbeddingTable(table)) {
      const relevantCodeCountContext = `${EVENT_NAME.voidGlobalContext}-${EXTENSION_CONTEXT_NAME.voidRelevantCodeSnippets}`
      const stored = this.context?.globalState.get(
