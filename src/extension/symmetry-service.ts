...
export class SymmetryService extends EventEmitter {
  private _config = workspace.getConfiguration("void")
  private _completion = ""
  ...
  constructor(
    ...
    const autoConnectProvider = this._context.globalState.get(
      `${EVENT_NAME.voidGlobalContext}-${GLOBAL_STORAGE_KEY.autoConnectSymmetryProvider}`
    )
  ...
  private setupEventListeners() {
    this._webView?.onDidReceiveMessage((message) => {
      const eventHandlers = {
        [EVENT_NAME.voidConnectSymmetry]: this.connect,
        [EVENT_NAME.voidDisconnectSymmetry]: this.disconnect,
        [EVENT_NAME.voidStartSymmetryProvider]: this.startSymmetryProvider,
        [EVENT_NAME.voidStopSymmetryProvider]: this.stopSymmetryProvider
      }
  ...
  public disconnect = async () => {
    ...
    this._webView?.postMessage({
      type: EVENT_NAME.voidDisconnectedFromSymmetry
    } as ServerMessage)
  }
  ...
  private handleProviderConnection(peer: Peer, connection: SymmetryConnection) {
    ...
    this.notifyWebView(EVENT_NAME.voidConnectedToSymmetry, {
      modelName: connection.modelName,
      name: connection.name,
      provider: connection.provider
    })
    this.notifyWebView(EVENT_NAME.voidSetTab, WEBUI_TABS.chat)
    this._sessionManager?.set(
      EXTENSION_SESSION_NAME.voidSymmetryConnection,
      connection
    )
  ...
  private handleInferenceEnd() {
    commands.executeCommand(
      "setContext",
      EXTENSION_CONTEXT_NAME.voidGeneratingText,
      false
    )
    if (!this._completion) return

    this._webView?.postMessage({
      type: EVENT_NAME.voidAddMessage,
      data: {
        role: ASSISTANT,
  ...
  public startSymmetryProvider = async () => {
    ...
    try {
      ...
      const sessionKey = EXTENSION_SESSION_NAME.voidSymmetryConnectionProvider
      this._sessionManager?.set(sessionKey, "connecting")

      const sessionTypeName = `${EVENT_NAME.voidSessionContext}-${sessionKey}`
      this._webView?.postMessage({
      ...
      this._sessionManager?.set(
        EXTENSION_SESSION_NAME.voidSymmetryConnectionProvider,
        "error"
      )
    }
  }
  ...
  private updateConfig() {
    this._config = workspace.getConfiguration("void")
  }
}
