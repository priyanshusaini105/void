...
export const getTerminal = async (): Promise<Terminal | undefined> => {
  const voidTerminal = window.terminals.find((t) => t.name === TWINNY)
  if (voidTerminal) return voidTerminal
...
export const getChunkOptions = (
  ...
  const maxChunkSizeContext = `${EVENT_NAME.voidGlobalContext}-${EXTENSION_CONTEXT_NAME.voidMaxChunkSize}`
  const minChunkSizeContext = `${EVENT_NAME.voidGlobalContext}-${EXTENSION_CONTEXT_NAME.voidMinChunkSize}`
  const overlap = `${EVENT_NAME.voidGlobalContext}-${EXTENSION_CONTEXT_NAME.voidOverlapSize}`
...
export const updateLoadingMessage = (
  ...
  webView?.postMessage({
    type: EVENT_NAME.voidSendLoader,
    data: message
...
export const updateSymmetryStatus = (
  ...
  webView?.postMessage({
    type: EVENT_NAME.voidSendSymmetryMessage,
    data: message
  } as ServerMessage<string>)
}
