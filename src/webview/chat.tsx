...
export const Chat = (props: ChatProps): JSX.Element => {
  ...
  const messageEventHandler = (event: MessageEvent) => {
    ...
    switch (message.type) {
      ...
      case EVENT_NAME.voidStopGeneration: {
        setIsLoading(false)
        setCompletion(null)
        stopRef.current = false
        generatingRef.current = false
  ...
  const handleRegenerateMessage = (
    ...
    setMessages((prev) => {
      ...
      global.vscode.postMessage({
        type: EVENT_NAME.voidChatMessage,
  ...
  useEffect(() => {
    global.vscode.postMessage({
      type: EVENT_NAME.voidHideBackButton
    })
  }, [])

  useEffect(() => {
    if (editorRef.current) {
      global.vscode.postMessage({ type: EVENT_NAME.voidSidebarReady })
    }
  }, [editorRef.current])
