...
export const useSymmetryConnection = () => {
  ...
  const connectToSymmetry = () => {
    ...
    global.vscode.postMessage({
      type: EVENT_NAME.voidConnectSymmetry,
      data: selectedModel
  ...
  const disconnectSymmetry = () => {
    setConnecting(true)
    global.vscode.postMessage({
      type: EVENT_NAME.voidDisconnectSymmetry
    } as ClientMessage)
  }

  const connectAsProvider = () => { // Removed useCallback
    global.vscode.postMessage({
      type: EVENT_NAME.voidStartSymmetryProvider
    } as ClientMessage)
  }

  const disconnectAsProvider = () => {
    global.vscode.postMessage({
      type: EVENT_NAME.voidStopSymmetryProvider
    } as ClientMessage)
  }

  const getModels = () => {
    global.vscode.postMessage({
      type: EVENT_NAME.voidGetSymmetryModels
    })
  }
  ...
  const handler = (event: MessageEvent) => {
    ...
    > = event.data
    if (message?.type === EVENT_NAME.voidConnectedToSymmetry) {
      setConnecting(false)
      setSymmetryConnectionSession(message.data as SymmetryConnection)
    }
    if (message?.type === EVENT_NAME.voidDisconnectedFromSymmetry) {
      setConnecting(false)
      setSymmetryConnectionSession(undefined)
    }
    if (message?.type === EVENT_NAME.voidSendSymmetryMessage) {
      setSymmetryProviderStatus(message?.data as string)
    }

    if (message?.type === EVENT_NAME.voidSymmetryModels) {
